...
def encode(msgType, payload):
    raw_bytes = bytearray(5 + len(payload))
    ...
    raw_bytes[len(payload) + 3] = (cksum1 & 0xFF)
    raw_bytes[len(payload) + 4] = (cksum2 & 0xFF)

    return raw_bytes


def decode(raw_bytes):
    if raw_bytes[0] != HEADER1 and raw_bytes[1] != HEADER2:
        return

    cmd = raw_bytes[2]

    if cmd != 12:
        logging.debug("Non event notification message:%s" % raw_bytes)
        return

    msgType = raw_bytes[4]
    payloadIn = raw_bytes[5:]

    return Message(msgType, payloadIn)


def encodeEventData(payload):
    raw_bytes = bytearray(len(payload) + 1)
    raw_bytes[0] = len(payload) + 1

    for i in range(len(payload)):
        raw_bytes[i + 1] = payload[i] & 0xff

    return encode(12, raw_bytes)
